...
export default function DraftPage() {
  ...
  useEffect(() => {
    if (isInitialized && players.length === 0) {
      const allPlayers = [
        ...
        ...(defData as BasePlayer[]).map((p) => ({ ...p, position: 'DEF' as const })),
        ...(midData as BasePlayer[]).map((p) => ({ ...p, position: 'MID' as const })),
        ...(fwdData as BasePlayer[]).map((p) => ({ ...p, position: 'FWD' as const })),
      ] as Player[];
  ...
  const handlePlayerRowClick = (player: Player) => {
    ...
    const draftPlayer = players.find(p => 
      p.web_name === player.web_name && p.team === player.team
    );
    if (draftPlayer) {
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center">
          ...
          <div className="text-muted-foreground">Setting up your draft environment</div>
        </div>
      </div>
    );
  }
  ...
  const tablePlayer = allPlayers.map(originalPlayer => {
    const draftPlayer = players.find(dp => 
      dp.web_name === originalPlayer.web_name && dp.team === originalPlayer.team
    );
